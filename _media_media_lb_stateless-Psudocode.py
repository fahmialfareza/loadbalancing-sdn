...
CLASS loadbalancer(app_manager.RyuApp):
    ...
    FUNCTION _packet_in_handler(self, ev):
        ...
        IF tcp_header.dst_port = 80:
            ...
            flow_mod <- parser.OFPFlowMod(datapath=datapath, match=match, idle_timeout=0, instructions=inst, buffer_id=msg.buffer_id, cookie=cookie)
            datapath.send_msg(flow_mod)
            match <- parser.OFPMatch(in_port=server_outport_selected, eth_type=eth.ethertype, eth_src=server_mac_selected, eth_dst=eth.src, ip_proto=ip_header.proto, ipv4_src=server_ip_selected, ipv4_dst=ip_header.src)
            ...
            cookie <- random.randint(0, 0xffffffffffffffff)
            flow_mod2 <- parser.OFPFlowMod(datapath=datapath, match=match, idle_timeout=0, instructions=inst2, cookie=cookie)
